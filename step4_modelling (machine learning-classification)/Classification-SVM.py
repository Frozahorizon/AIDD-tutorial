...
#
def svm_model(features, target, C, kernel):
    print("The SVM model is training, the parameters of svm was optimized.")
    X_train, X_test, y_train, y_test = partition(feature_data=features, target_data=target)
    random_seed = np.random.RandomState(0)
    classifier = SVC(C=C, kernel=kernel, gamma='scale', probability=True, random_state=random_seed)
    classifier.fit(X_train, y_train)
    ...
    with open('./SVM_output/SVM_model.pkl', 'wb') as file:
    ...
    plt.savefig('./SVM_output/ROC_AUC.png', dpi=600)
    plt.show()


svm_model(features=features, target=target, C=1, kernel='rbf')
