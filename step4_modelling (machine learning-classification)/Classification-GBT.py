...
################################################
# create a new folder to save the output file
try:
    os.makedirs('GBT_output')
...
################################################
# using GridSearchCV to optimization the parameters
...
def gbt_model(features, target, loss='deviance', learning_rate=1, n_estimators=300):
    print("The GBT model is training, the parameters of svm was optimized.")
    X_train, X_test, y_train, y_test = partition(feature_data=features, target_data=target)
    ...
    with open('./GBT_output/GBT_model.pkl', 'wb') as file:
    ...
    plt.savefig('./GBT_output/ROC_AUC.png', dpi=600)
    plt.show()


gbt_model(features=features, target=target)
