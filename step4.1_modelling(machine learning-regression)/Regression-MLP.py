...
hidden_layer_sizes = [(i,) for i in range(40, 200, 20)]
...
def parameter_optimization(features_data, target_data, m_scores=optimization_scores, parameter_grid=param_grid):
    ...
    base_estimator = MLPRegressor(random_state=random_seed)
...
################################################
def mlp_model(features_data, target_data, hls=(100,)):
    ...
    with open('./MLP_output/MLP_model.pkl', 'wb') as file:
        pickle.dump(regressor, file)


mlp_model(features_data=features, target_data=target)

################################################
# 10-fold cross validation
def mlp_model_10fold(features_data, target_data, hls):
    ...
    name = ['{0}'.format('MLP')] * 10
    name_df = pd.DataFrame(name)

    MS = pd.concat([name_df, scores_df], axis=1)
    MS.columns = ['Model', 'Scores']
    MS.to_excel('./MLP_output/MLP_Scores.xlsx', index=False)

    sns.boxplot(x='Model', y='Scores', color='#00b8e5', data=MS)
    plt.savefig("./MLP_output/boxplot.jpg", dpi=600)
    plt.show()


# mlp_model_10fold(features_data=features, target_data=target, hls=(100,))
################################################
# record time
